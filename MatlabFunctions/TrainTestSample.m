...
% Outputs
%   X_tr, X_ts - Trainng and testing featrue matrices
%   Y_tr, Y_ts - Labels of trainig and testing sets 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [X_tr, Y_tr, X_ts, Y_ts] = TrainTestSample(X_control, X_case, tr_per)
X_c2 = X_case; 
X_n2 = X_control;

% asign labels case and control as 1 and 0
y_c = repelem(1, size(X_c2,1));
y_n = repelem(0,size(X_n2,1)); 

% select number of samples for fitting a classification model
A  = min(length(y_c), length(y_n));

% number of model training and testing samples     
Train = round(A *tr_per); 
Test = A - Train; 
